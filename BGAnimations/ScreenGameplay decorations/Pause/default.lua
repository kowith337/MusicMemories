...
local Inputne = function( event )
	...
	if event.type == "InputEventType_FirstPress" then
		...
		if (event.button == "MenuLeft") and (event.PlayerNumber == PLAYER_1 or event.PlayerNumber == PLAYER_2) then
			Karkao[1] = 1
		end
		if (event.button == "MenuRight") and (event.PlayerNumber == PLAYER_1 or event.PlayerNumber == PLAYER_2) then
			Karkao[2] = 1
		end
