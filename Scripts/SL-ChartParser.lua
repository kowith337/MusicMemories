...
local function GetSimfileChartString(SimfileString, StepsType, Difficulty, Filetype, des)
	...
	if Filetype == "ssc" then
		...
		for chart in SimfileString:gmatch("#NOTEDATA.-#NOTES:[^;]*") do
			...
			if(chart:match("#STEPSTYPE:"..regexEncode(StepsType)) and chart:match("#DIFFICULTY:"..regexEncode(Difficulty)) and chart:match("#DESCRIPTION:"..regexEncode(des))) then
				--Find just the notes and remove comments
				measuresString = chart:match("#NOTES:[\r\n]+([^;]*)\n?$"):gsub("//[^\r\n]*","") .. ";"
			elseif(chart:match("#STEPSTYPE:"..regexEncode(HelpAutogen(StepsType,1))) and chart:match("#DIFFICULTY:"..regexEncode(Difficulty)) and chart:match("#DESCRIPTION:"..regexEncode(des))) then
				measuresString = chart:match("#NOTES:[\r\n]+([^;]*)\n?$"):gsub("//[^\r\n]*","") .. ";"
			elseif (chart:match("#STEPSTYPE:"..regexEncode(StepsType)) and chart:match("#DIFFICULTY:"..regexEncode(Difficulty))) then
				--Find just the notes and remove comments
				measuresString = chart:match("#NOTES:[\r\n]+([^;]*)\n?$"):gsub("//[^\r\n]*","") .. ";"
				--else
...
local function getNoteMeasures(measuresString)
	...
	do
		...
		if(line:match("^[,;]%s*")) then
			...
			table.insert(NoteMeter, #measureNotes)
...
local function Note2NPS(Noto,pn)
	
	local Nepu = GAMESTATE:GetCurrentSteps(pn):GetTimingData()
	local may = {}
	
	for i,m in pairs(Noto) do
		table.insert(may,(m/(Nepu:GetElapsedTimeFromBeat((i+1)*4)-Nepu:GetElapsedTimeFromBeat(i*4))))
	end
...
function AvofTable(hay)
	local sum = 0;
	
	for i,m in pairs(hay) do
		sum = sum + m
	end
...
local function getStreamSequences(NoteMeasures, measureSequenceThreshold, NPM, Pn)
	local streamMeasures = {}
	
	if NPM == 6969 then
		local NPSs
		NPSs = Note2NPS(NoteMeasures,Pn)
		AvNps = AvofTable(NPSs)
		for i,m in pairs(NPSs) do
			if m >= AvNps then
				table.insert(streamMeasures,i)
			end
		end
	else
		for i,m in pairs(NoteMeasures) do
			if m >= NPM then
				table.insert(streamMeasures,i)
			end
		end
...
function GetStreams(SongDir, StepsType, Difficulty, NotesPerMeasure, MeasureSequenceThreshold,Players,moree)

	local SimfileString, Filetype = GetSimfileString( SongDir )
	if not SimfileString then return end

	-- Parse out just the contents of the notes
	local ChartString = GetSimfileChartString(SimfileString, StepsType, Difficulty, Filetype,moree)
	if not ChartString then return end

	-- Which measures have enough notes to be considered as part of a stream?
	local NoteMeasures = getNoteMeasures(ChartString)

	-- Which sequences of measures are considered a stream?
	return (getStreamSequences(NoteMeasures, MeasureSequenceThreshold, NotesPerMeasure,Players))
end